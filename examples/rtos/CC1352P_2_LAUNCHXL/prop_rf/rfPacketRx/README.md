...
When running the secure non-secure configuration (tfm_rfPacketRx), follow the steps below.

1. Load the secure image `tfm_s.axf` located in `<SDK_INSTALL_DIR>/tfm_s/build/cc26x4/production_full/Release/outputs/CC26X4/`
2. Load the non-secure image by adding symbols for tfm_rfPacketRx.out
3. Run the example
